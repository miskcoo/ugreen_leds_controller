...
- **TrueNAS**: see [#13](https://github.com/miskcoo/ugreen_leds_controller/issues/13) (and maybe [here](https://github.com/miskcoo/ugreen_leds_controller/tree/truenas-build/build-scripts/truenas)) for how to build the module, and [here](https://gist.github.com/Kerryliu/c380bb6b3b69be5671105fc23e19b7e8) for a script using the cli tool; [here](https://github.com/miskcoo/ugreen_leds_controller/tree/gh-actions/build-scripts/truenas/build) for pre-build drivers.
  - *NB see below for [important information regarding TrueNAS Scale installations and upgrades](#important-information-for-truenas-scale-users)*
...
##### Important information for TrueNAS Scale users

The installation and upgrade process for TrueNAS Scale overwrites all of

```
/etc/
/etc/modprobe.d/
/etc/modules-load.d/
/lib/modules/$(uname -r)/kernel/drivers/leds/
```

with those contained in the release build image and remounts the underlying filesystem as read only once installed, so [developer mode](https://www.truenas.com/docs/scale/scaletutorials/systemsettings/advanced/developermode/) must be enabled to both compile the kernel module (if you want to compile it on the UGREEN device) _and_ install it and all associated config files to ensure it loads on boot. Developer mode does not persist over upgrades for the same reasons.
